...
/*   Updated: 2024/12/16 20:38:22 by fmaurer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	tokenize_rout1(t_token *prev, t_token *cur, t_token *next)
{
	if (is_posint_numstr(prev->value) >= 0 && next->type == TOK_WORD)
...
static void	tokenize_routa1(t_token *prev, t_token *cur, t_token *next)
{
	if (is_posint_numstr(prev->value) >= 0 && \
			is_posint_numstr(next->value) >= 0)
	{
		prev->type = TOK_ROUTA_FILDES_IN;
		next->type = TOK_ROUTA_FILDES_OUT;
	}
	else if (is_posint_numstr(prev->value) >= 0 && \
			is_posint_numstr(next->value) == -1)
	{
		prev->type = TOK_ROUTA_FILDES_IN;
		next->type = TOK_OF;
	}
